...
async def is_subscribed(context, user_id):
    try:
        # গ্রুপ মেম্বার কি না তা চেক করবে
        member = await context.bot.get_chat_member(chat_id=GROUP_CHAT_ID, user_id=user_id)
        # যদি ইউজার গ্রুপে থাকে (মেম্বার, অ্যাডমিন বা ক্রিয়েটর হয়)
        return member.status not in ['left', 'kicked']
    except Exception as e:
        logging.error(f"Membership check error: {e}")
        # যদি কোনো ভুল হয় (যেমন বট অ্যাডমিন না), তবে সেফ থাকার জন্য False দিবে
        return False
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # গ্রুপে জয়েন না থাকলে শুধু সাবস্ক্রিপশন বাটন দেখাবে
