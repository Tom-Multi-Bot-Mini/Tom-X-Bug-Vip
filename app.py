...
# Bot must be admin in these channels/groups
CHANNEL_ID = "@your_channel" 
GROUP_ID = "@https://t.me/tomxbugvip"
...
def get_main_menu(user_id):
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("║ Channel ║", url=f"https://youtube.com/@saycotom?si=nlptFT57FmTMfDrY")]
    ]
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not await is_subscribed(context, user_id):
        sub_text = "❌ **You must subscribe my telegram channel to use this bot.**"
        sub_kb = InlineKeyboardMarkup([
            [InlineKeyboardButton("📢 Join Channel", url=f"https://youtube.com/@saycotom?si=nlptFT57FmTMfDrY")],
...
async def conect_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id not in VIP_USERS:
        # Access Denied layout from screenshot
        denied_text = (
            "🚫 **Access Denied**\n\n"
...
async def handle_click(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "btn_bug":
        # All Bug commands from screenshot
        bug_text = (
            "┏━━━━━━ **TOM-X BUG MENU** ━━━━━━\n"
            "┣━━━━━━ **𝐏𝐎𝐊 𝐏𝐎𝐊 𝐀𝐍𝐃𝐑𝐎𝐈𝐃** ━━━━━━\n"
            "┃ ➤ tomcsdroid **num time**\n"
        ...
        # All Misc commands from screenshot
        misc_text = (
            "┌────┤ **Misc Menu** ├────────┐\n"
            "│➤ conect **number**\n"
            "│➤ delconect **number**\n"
    ...
    elif query.data == "home" or query.data == "verify_sub":
        text, reply_markup = get_main_menu(user_id)
        try: await query.edit_message_caption(caption=text, reply_markup=reply_markup, parse_mode='Markdown')
        except: await query.message.reply_text(text, reply_markup=reply_markup, parse_mode='Markdown')
