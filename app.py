...
# ৫ নম্বর লাইনের ভুল ইম্পোর্টটি ঠিক করা হয়েছে
...
async def handle_commands(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # WhatsApp Attack Logic (Using Node.js)
    attack_cmds = ['tomcsdroid', 'tomjam', 'tomcut', 'tomcs', 'tomoff', 'tomsysgp', 'hangui']
    
    # /reqpair কমান্ডটি হ্যান্ডেল করা
    if cmd == "/reqpair":
        if len(args) < 2: return await update.message.reply_text("❌ Usage: `/reqpair +880...`")
        subprocess.Popen(["node", "pair.js", args[1]])
        await update.message.reply_text(f"⏳ **Pairing Code Request Sent.** Check Render Logs!")
...
# --- ৪. বাটন ক্লিক হ্যান্ডলার (সব মেনু যোগ করা হয়েছে) ---
...
def main():
    ...
    app.add_handler(CommandHandler("reqpair", handle_commands)) # reqpair হ্যান্ডলার যোগ করা হলো
    app.add_handler(CallbackQueryHandler(handle_click))
    app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, handle_commands))
    
    print("Tom-X Ultimate System is Live...")
    app.run_polling()
